...
bool StateConnected::frequencyTimerOne(uint8_t p_delta)
{
	float fdelta = static_cast<float>(p_delta);
...
bool StateConnected::frequencyTimerTwo(uint8_t p_delta)
{
	float fdelta = static_cast<float>(p_delta);
...
void StateConnected::SendAndReceive(uint8_t p_delta)
{
						...
						if (messageType == LOCAL_ENTITY_MESSAGE)
						{
							...
							for (auto& inputPredictionStruct : inputPredictionStructs)
							{
								...
								if (inputPredictionStruct->uniqueID == positionMessage->uniqueID)
								{
									...
									RecalculateCurrentPosition(*positionMessage, pos); //skicka vart iterorn är
									break;
								}
								pos++;
							}
							DeleteOldPredictions(positionMessage->uniqueID);
...
//Input predictions with a lot of help from Christian Edberg.
...
//Input predictions with a lot of help from Christian Edberg.
void StateConnected::RecalculateCurrentPosition(local_entity_state message, int p_currentPositionIndex)
{
	auto it = inputPredictionStructs.begin() + p_currentPositionIndex;
	inputPredictionStruct* previousHistory = *it;

	for (auto itr = it; itr != inputPredictionStructs.end(); itr++)
	{
		if (itr == it) {
			(*itr)->PosX = message.positionX * scale;
			(*itr)->PosY = message.positionY * scale;
			previousHistory = *itr;
			if (*itr == inputPredictionStructs.back())
			{
				tanks[myID]->SetTankPos((*itr)->PosX, (*itr)->PosY);
				break;
			}
		}
		else
		{
			CreateNewInputPredictionElement(*previousHistory, *(*itr), previousHistory->deltaTime);
			if (*itr == inputPredictionStructs.back()) {
				tanks[myID]->SetTankPos((*itr)->PosX, (*itr)->PosY);
				break;
			}
...
//Input predictions with a lot of help from Christian Edberg.
void StateConnected::DeleteOldPredictions(uint16_t uniqueID)
{
	auto it = inputPredictionStructs.begin();
	while (it != inputPredictionStructs.end())
	{
	...
	inputPredictionStructs.erase(std::remove(inputPredictionStructs.begin(), inputPredictionStructs.end(), nullptr), inputPredictionStructs.end());
}

//Input predictions with a lot of help from Christian Edberg.
void StateConnected::CreateNewInputPredictionElement(inputPredictionStruct& a, inputPredictionStruct& b, uint8_t deltaTime)
{
