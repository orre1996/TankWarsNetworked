...
server_send_payload(server &srv, connection *conn, const int64_t current_tick)
{
	...
	MakeLocalEntityMessage(*conn, srv);
	MakeRemoteEntityMessage(*conn,srv);
	MakeGameStateMessage(*conn, srv);
	MakeKillMessage(*conn, srv);
	MakeRoundTripMessage(*conn, srv);
	MakeConnectMessage(*conn, srv);
	MakeDisconnectMessage(*conn, srv);
	SerializeMessage(conn, srv, current_tick);
}

void MakeLocalEntityMessage(connection& p_conn, server& p_srv)
{
	auto tanks = p_srv.gameServer->GetTanks();

	for (auto i = 0; i < tanks.size(); i++)
	{
		if (tanks[i]->getEntityID() == p_conn.index && p_conn.client_salt_ != 0)
		{
			auto* pMessage = new Local_Entity_State();
			make_local_entity_message(*pMessage, tanks[i]->GetTankX(),
				tanks[i]->GetTankY(), p_conn.lastReceivedUniqueID, tanks[i]->getEntityID(), tanks[i]->getEntityState());
			p_conn.msgQueue.push_back(pMessage);
			break;
		}
	}
...
{
	auto entities = p_srv.gameServer->GetTanksAndBullets();
	for (auto i = 0; i < p_srv.gameServer->GetTanksAndBullets().size(); i++)
	{
		if (entities[i]->getEntityID() == p_conn.index)
		{
			continue;
		}
...
void MakeGameStateMessage(connection& p_conn, server& p_srv)
{
	...
	p_srv.iDArray[p_conn.index] = p_conn.index;
	p_srv.scoreArray[p_conn.index] = p_conn.score;
	p_srv.roundtripTimer[p_conn.index] = p_conn.lastReceivedRTT;

	const auto gsMessage = new game_state_message();
	make_game_state_message(*gsMessage, p_srv.gameServer->GetCurrentGameState(), p_srv.iDArray, p_srv.scoreArray, p_srv.roundtripTimer, p_srv.gameServer->getRoundTimer() / 1000);
	p_conn.msgQueue.push_back(gsMessage);
}

void MakeKillMessage(connection& p_conn, server& p_srv)
{
	if (p_srv.gameServer->GetKillMessageTries() < 10)
	{
		auto* kMessage = new kill_message();
		make_kill_message(*kMessage, p_srv.gameServer->GetKillID(), p_srv.gameServer->GetVictimID());
		p_srv.gameServer->IncreaseKillMessageTries();
		p_conn.msgQueue.push_back(kMessage);
	}
}
...
void MakeRoundTripMessage(connection& p_conn, server& p_srv)
{
	...
	p_conn.msgQueue.push_back(rttMessage);
}

void MakeConnectMessage(connection& p_conn, server& p_srv)
{
	const uint8_t connections_count = p_srv.connection_count_;

	if (static_cast<int>(p_srv.connections_index) < static_cast<int>(connections_count))
	{
		if (p_srv.sendConnectionsMessageTries < 10)
		{
			const auto cMessage = new Connected_Message();
			make_connected_message(*cMessage,p_srv.gameServer->getLatestConnect());
			p_conn.msgQueue.push_back(cMessage);
			p_srv.sendConnectionsMessageTries++;
		}
		else
		{
			p_srv.connections_index++;
		}
	}
}
...
void MakeDisconnectMessage(connection& p_conn, server& p_srv)
{
	...
	if (static_cast<int>(p_srv.connections_index) > static_cast<int>(connections_count))
	{
		if (p_srv.sendDisonnectionsMessageTries < 10)
		{
			const auto dcMessage = new Disconnected_Message();
			make_disconnected_message(*dcMessage, p_srv.gameServer->getLatestDc());
			p_conn.msgQueue.push_back(dcMessage);
			p_srv.sendDisonnectionsMessageTries++;
		}
		else
		{
			p_srv.connections_index--;
...
void SerializeMessage(connection* p_conn, server& p_srv, const int64_t current_tick)
{
	...
	while (!p_conn->msgQueue.empty())
	{
		...
		{
			const auto cm = static_cast<Connected_Message*>(msg);
		...
		{
			const auto dcm = static_cast<Disconnected_Message*>(msg);
		...
		{
			const auto gsm = static_cast<game_state_message*>(msg);
		...
		{
			const auto lm = static_cast<Local_Entity_State*>(msg);
		...
		{
			const auto rm = static_cast<Remote_Entity_State*>(msg);
		...
		{
			const auto rtt = static_cast<roundtriptimer_message*>(msg);
		...
		{
			const auto km = static_cast<kill_message*>(msg);
                                  ...
                                  const int64_t current_tick)
{
   ...
   if (!ch)
   {
	...
	const uint64_t challenge_salt = ch->client_salt_ ^ packet.combined_salt_;
                       ...
                       const int64_t current_tick)
{
   ...
   if (conn->latestSequence <= packet.sequence_)
   {
	   ...
	   srv.gameServer->on_payload(conn->index, &packet, *conn, srv.deltaTime);
   }
}
...
void server_run(server &srv)
{
   ...
   while (true)
   {
	  ...
	  srv.lastTick = time;
	  const float floatDelta = static_cast<float>(srv.deltaTime) * 0.001f;

	  auto bullets = srv.gameServer->getBullets();
	  auto tanks = srv.gameServer->GetTanks();

	  for (auto& bullet : bullets)
	  {
		  bullet->Update(floatDelta);
	  }
	 
	  if (srv.gameServer->GetCurrentGameState() == RUNNING)
	  {
		  for (auto& tank : tanks)
		  {
			  tank->Update(floatDelta);
		  }
	  }
